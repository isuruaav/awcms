<div class="mx-auto max-w-7xl space-y-6">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        <a
            href="{{ route('admin.news.preview', $news) }}"
            target="_blank"
                ...
                <div class="mt-2 flex flex-wrap gap-2">
                    @foreach ($locales as $localeOption)
                        ...
                        @if ($translation instanceof \App\Models\News)
                            <a
                                href="{{ $translation->status === \App\Enums\NewsStatus::Draft
                                ...
                                @class([
                                    ...
                                    'bg-zinc-100 text-zinc-700 hover:bg-zinc-200' => $translation->id !== $news->id,
                                ])
                            >
                        ...
                        @else
                            @can('news.create')
                                <a
                                    href="{{ route('admin.news.translations.create', ['newsId' => $news->id, 'locale' => $localeOption->value]) }}"
                                    wire:navigate
    ...
    <form wire:submit="save" class="space-y-6">
        <section class="rounded-2xl border border-zinc-200 bg-white shadow-sm">
            ...
            <div class="grid gap-5 p-6 lg:grid-cols-2">
                <div>
                    <label for="news-title" class="mb-2 block text-sm font-semibold text-zinc-800">News title</label>
                    <input
                        id="news-title"
                        type="text"
                ...
                <div>
                    <label for="news-category" class="mb-2 block text-sm font-semibold text-zinc-800">Category</label>
                    <select
                        id="news-category"
                    ...
                    @error('categoryId')<p class="mt-2 text-sm font-medium text-red-600">{{ $message }}</p>@enderror
                </div>

                <div class="lg:col-span-2">
                    <div class="mb-2 flex items-center justify-between gap-3">
                        <label for="news-slug" class="text-sm font-semibold text-zinc-800">URL slug</label>
                        <button type="button" wire:click="regenerateSlug" class="text-xs font-semibold text-emerald-700 hover:text-emerald-800">Generate from title</button>
                    </div>
                    ...
                    <div class="flex overflow-hidden rounded-xl border border-zinc-300 focus-within:border-emerald-500 focus-within:ring-4 focus-within:ring-emerald-500/10">
                        ...
                        <input id="news-slug" type="text" wire:model.live.debounce.300ms="slug" class="min-w-0 flex-1 border-0 bg-white px-4 py-3 text-sm outline-none focus:ring-0">
                    </div>
                    @error('slug')<p class="mt-2 text-sm font-medium text-red-600">{{ $message }}</p>@enderror
                </div>
                ...
                <div class="lg:col-span-2">
                    ...
                    ></textarea>
                    @error('summary')<p class="mt-2 text-sm font-medium text-red-600">{{ $message }}</p>@enderror
                </div>
            </div>
        </section>
        ...
        <section class="rounded-2xl border border-zinc-200 bg-white shadow-sm">
            <div class="border-b border-zinc-200 px-6 py-5">
                <div class="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-zinc-600">Visual Editor and HTML + Tailwind use the same saved HTML.</p>
                    </div>
                    <span class="w-fit rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-800">Visual + Code</span>
                </div>
            </div>
            ...
            <div class="p-6">
                ...
                />
                @error('content')<p class="mt-3 text-sm font-medium text-red-600">{{ $message }}</p>@enderror
            </div>
        </section>
        ...
        <section class="rounded-2xl border border-zinc-200 bg-white shadow-sm">
            ...
            <div class="space-y-6 p-6">
                            ...
                            <article class="overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-sm">
                                <div class="aspect-[4/3] overflow-hidden bg-zinc-100">
                                    @if ($newsImageUrl)
                                        ...
                                        >
                                    @else
                                        <div class="flex h-full items-center justify-center px-4 text-center text-xs font-semibold text-zinc-400">
                                            Preview unavailable
                                        </div>
                                    @endif
                                </div>
                ...
                @else
                    <div class="rounded-xl border-2 border-dashed border-zinc-200 bg-zinc-50 px-5 py-8 text-center">
                ...
                @if (auth()->user()?->can('media.upload') && auth()->user()?->can('news.update'))
                    <div class="border-t border-zinc-200 pt-5">
                        ...
                        <p class="mt-2 text-xs leading-5 text-zinc-500">
                            Select up to 20 images at once, maximum 8 MB each. Uploaded files are stored through the existing secure Media Library pipeline.
                        </p>
        ...
        <section class="rounded-2xl border border-zinc-200 bg-white shadow-sm">
            ...
            <div class="grid gap-5 p-6 lg:grid-cols-2">
                <div>
                    <label for="featured-image" class="mb-2 block text-sm font-semibold text-zinc-800">Featured image</label>
                    <select
                        id="featured-image"
                    ...
                    >
                        ...
                        @foreach ($images as $image)
                            <option value="{{ $image->id }}">{{ $image->title ?: $image->original_name }} (#{{ $image->id }})</option>
                        @endforeach
                    </select>
                    <p class="mt-2 text-xs text-zinc-500">Only Public Media Library images are listed.</p>
                    @error('featuredImageId')<p class="mt-2 text-sm font-medium text-red-600">{{ $message }}</p>@enderror
                </div>

                <div>
                    <label for="published-at" class="mb-2 block text-sm font-semibold text-zinc-800">Publication date/time</label>
                    <input
                        id="published-at"
                        type="datetime-local"
                        ...
                        class="w-full rounded-xl border border-zinc-300 bg-white px-4 py-3 text-sm outline-none focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10"
                    >
                    <p class="mt-2 text-xs text-zinc-500">Blank uses the current time when Publish is clicked. Future dates schedule public visibility.</p>
                    @error('publishedAt')<p class="mt-2 text-sm font-medium text-red-600">{{ $message }}</p>@enderror
                </div>

                <label class="lg:col-span-2 flex cursor-pointer items-start gap-3 rounded-xl border border-zinc-200 bg-zinc-50 p-4">
                    <input type="checkbox" wire:model="isFeatured" class="mt-0.5 h-4 w-4 rounded border-zinc-300 text-emerald-700 focus:ring-emerald-500">
                    <span>
                        <span class="block text-sm font-semibold text-zinc-900">Featured news article</span>
                        <span class="mt-1 block text-xs leading-5 text-zinc-500">Featured articles may be prioritised in public news listings.</span>
                    </span>
                </label>
            </div>
        </section>
        ...
        <div class="flex flex-col-reverse gap-3 border-t border-zinc-200 pt-6 sm:flex-row sm:items-center sm:justify-end">
            <a href="{{ route('admin.news.index') }}" wire:navigate class="inline-flex items-center justify-center rounded-xl border border-zinc-300 bg-white px-5 py-3 text-sm font-semibold text-zinc-700 hover:bg-zinc-50">Cancel</a>
