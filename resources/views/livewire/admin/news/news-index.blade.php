    ...
    <section class="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
        <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-6">
            ...
            <div>
                ...
                <select id="news-locale" wire:model.live="locale" class="w-full rounded-xl border border-zinc-300 bg-white px-4 py-2.5 text-sm">
            ...
            <div>
                ...
                <select id="news-category" wire:model.live="category" class="w-full rounded-xl border border-zinc-300 bg-white px-4 py-2.5 text-sm">
            ...
            <div>
                ...
                <select id="news-status" wire:model.live="status" class="w-full rounded-xl border border-zinc-300 bg-white px-4 py-2.5 text-sm">
            ...
            <div>
                ...
                <select id="news-records" wire:model.live="recordState" class="w-full rounded-xl border border-zinc-300 bg-white px-4 py-2.5 text-sm">
        ...
        <div class="mt-4 flex flex-col gap-3 border-t border-zinc-100 pt-4 sm:flex-row sm:items-center sm:justify-between">
            <button type="button" wire:click="resetFilters" class="w-fit text-sm font-semibold text-emerald-700 hover:text-emerald-800">Reset filters</button>

            <div class="flex items-center gap-2">
                <label for="news-per-page" class="text-sm text-zinc-500">Rows</label>
                <select id="news-per-page" wire:model.live="perPage" class="rounded-lg border border-zinc-300 bg-white px-3 py-2 text-sm">
                ...
                <thead class="bg-zinc-50">
                    <tr>
                        <th class="px-5 py-3 text-left">
                            <button type="button" wire:click="sort('title')" class="text-xs font-semibold uppercase tracking-wide text-zinc-600 hover:text-zinc-900">
                                Article
                                @if ($sortField === 'title') {{ $sortDirection === 'asc' ? '↑' : '↓' }} @endif
                            </button>
                        </th>
                        <th class="px-5 py-3 text-left">
                            <button type="button" wire:click="sort('locale')" class="text-xs font-semibold uppercase tracking-wide text-zinc-600 hover:text-zinc-900">
                                Language
                                @if ($sortField === 'locale') {{ $sortDirection === 'asc' ? '↑' : '↓' }} @endif
                            </button>
                        </th>
                        <th class="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wide text-zinc-600">Category</th>
                        <th class="px-5 py-3 text-left">
                            <button type="button" wire:click="sort('status')" class="text-xs font-semibold uppercase tracking-wide text-zinc-600 hover:text-zinc-900">
                                Status
                                @if ($sortField === 'status') {{ $sortDirection === 'asc' ? '↑' : '↓' }} @endif
                            </button>
                        </th>
                        <th class="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wide text-zinc-600">Author</th>
                        <th class="px-5 py-3 text-left">
                            <button type="button" wire:click="sort('updated_at')" class="text-xs font-semibold uppercase tracking-wide text-zinc-600 hover:text-zinc-900">
                                Updated
                                @if ($sortField === 'updated_at') {{ $sortDirection === 'asc' ? '↑' : '↓' }} @endif
                            </button>
                        </th>
                        <th class="px-5 py-3 text-left">
                            <button type="button" wire:click="sort('published_at')" class="text-xs font-semibold uppercase tracking-wide text-zinc-600 hover:text-zinc-900">
                                Published
                                @if ($sortField === 'published_at') {{ $sortDirection === 'asc' ? '↑' : '↓' }} @endif
                            </button>
                        </th>
                        <th class="px-5 py-3 text-right text-xs font-semibold uppercase tracking-wide text-zinc-600">Actions</th>
                    </tr>
                </thead>
                ...
                <tbody class="divide-y divide-zinc-100">
                    @forelse ($news as $article)
                        ...
                        <tr wire:key="news-{{ $article->id }}" class="hover:bg-zinc-50/70">
                                ...
                                <div class="max-w-lg">
                                    <div class="flex flex-wrap items-center gap-2">
                                        ...
                                        @if ($article->is_featured)
                                            <span class="rounded-full bg-amber-100 px-2 py-0.5 text-[11px] font-bold uppercase tracking-wide text-amber-700">Featured</span>
                                        @endif
                                    </div>

                                    <p class="mt-1 text-xs text-zinc-500">/{{ $article->locale->value }}/news/{{ $article->slug }}</p>
                                    ...
                                    <div class="mt-2 flex flex-wrap gap-1.5">
                                        @foreach ($locales as $localeOption)
                                            ...
                                            @elseif ($recordState !== 'trashed')
                                                @can('news.create')
                                                    <a
                                                        href="{{ route('admin.news.translations.create', ['newsId' => $article->id, 'locale' => $localeOption->value]) }}"
                                                        wire:navigate
                                    ...
                                    @if ($article->summary)
                                        <p class="mt-2 max-w-md text-sm text-zinc-600">{{ \Illuminate\Support\Str::limit($article->summary, 100) }}</p>
                                    @endif
                                </div>
                            </td>

                            <td class="whitespace-nowrap px-5 py-4">
                                <span class="inline-flex rounded-full bg-blue-50 px-2.5 py-1 text-xs font-bold text-blue-700">{{ $article->locale->nativeLabel() }}</span>
                                <p class="mt-1 text-[11px] font-semibold uppercase tracking-wide text-zinc-400">{{ $article->locale->value }}</p>
                            </td>

                            <td class="px-5 py-4">
                                @if ($article->category)
                                    <span class="inline-flex rounded-full bg-sky-50 px-2.5 py-1 text-xs font-semibold text-sky-700">{{ $article->category->name }}</span>
                                @else
                            ...
                            <td class="whitespace-nowrap px-5 py-4">
                                <span class="inline-flex rounded-full px-2.5 py-1 text-xs font-semibold {{ $statusClasses }}">{{ $article->status->label() }}</span>
                            </td>

                            <td class="whitespace-nowrap px-5 py-4">
                                <p class="text-sm font-medium text-zinc-800">{{ $article->creator?->name ?? 'Unknown' }}</p>
                            ...
                            <td class="px-5 py-4">
                                <div class="flex min-w-48 flex-wrap justify-end gap-2">
                                    ...
                                    @else
                                        @can('news.view')
                                            <a
                                                href="{{ route('admin.news.preview', $article) }}"
                                                target="_blank"
                                        ...
                                        @if ($article->status === \App\Enums\NewsStatus::Published)
                                            @can('news.publish')
                                                <button
                                                    type="button"
                                                    wire:click="unpublishArticle({{ $article->id }})"
                                        ...
                                        @else
                                            @can('news.publish')
                                                <button
                                                    type="button"
                                                    wire:click="publishArticle({{ $article->id }})"
