<div class="space-y-6">
    ...
    <div class="flex flex-wrap items-end justify-between gap-4">
        <div>
            ...
            <h1 class="mt-1 text-2xl font-black text-zinc-950">Past Commandants</h1>
    ...
    <div class="grid min-w-0 gap-6 xl:grid-cols-[minmax(0,1fr)_390px]">
        <section class="overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-sm">
            ...
            <div class="divide-y divide-zinc-200">
                @forelse ($pastCommandants as $commandant)
                    <article wire:key="past-commandant-{{ $commandant->id }}" class="min-w-0 p-4 sm:p-5">
                        <div class="flex min-w-0 items-start gap-4">
                            <div
                                ...
                                @if ($commandant->image)
                                    @php($imageUrl = $mediaUrls->thumbnailOrOriginal($commandant->image))
                                    @if ($imageUrl)
                                        <img src="{{ $imageUrl }}" alt="{{ $commandant->name_en }}"
                                            class="h-full w-full object-cover object-top">
                                    @else
                                        <span class="px-3 text-center text-xs font-semibold text-zinc-600">
                                            {{ $commandant->image->title ?: $commandant->image->original_name }}
                                        </span>
                                    @endif
                                @else
                                ...
                                <div class="mt-3 grid gap-1.5 text-xs text-zinc-500">
                                    <span class="inline-flex items-center gap-2">
                                        ...
                                        <span>
                                            ...
                                            <strong class="font-bold text-zinc-700">
                                                {{ $commandant->from_date->format('d.m.Y') }}
                                            </strong>
                                        </span>
                                    </span>
                                    ...
                                    <span class="inline-flex items-center gap-2">
                                        ...
                                        <span>
                                            To:
                                            <strong class="font-bold text-zinc-700">
                                                {{ $commandant->to_date->format('d.m.Y') }}
                                            </strong>
                                        </span>
                                    </span>
                                </div>
                ...
                @empty
                    <div class="px-6 py-14 text-center text-sm text-zinc-500">No past commandants have been added.</div>
                @endforelse
            </div>
        </section>
            ...
            <form wire:submit="save"
                ...
                <div class="border-b border-zinc-200 bg-zinc-50 px-6 py-4">
                    <h2 class="font-bold text-zinc-900">
                        {{ $editingId === null ? 'New Past Commandant' : 'Edit Past Commandant' }}</h2>
                </div>
                <div class="space-y-5 p-6">
                    <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-1">
                        <div><label for="past-name-en" class="mb-1 block text-xs font-bold text-zinc-700">English
                                name</label><input id="past-name-en" wire:model="nameEn" type="text" maxlength="180"
                                class="w-full rounded-lg border border-zinc-300 px-3 py-2.5 text-sm">
                            @error('nameEn')
                                <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        <div><label for="past-name-si" class="mb-1 block text-xs font-bold text-zinc-700">සිංහල
                                නම</label><input id="past-name-si" wire:model="nameSi" type="text" maxlength="220"
                    ...
                    <div class="grid gap-4 sm:grid-cols-2">
                        <div><label for="past-from-date" class="mb-1 block text-xs font-bold text-zinc-700">From
                                date</label><input id="past-from-date" wire:model="fromDate" type="date"
                                class="w-full rounded-lg border border-zinc-300 px-3 py-2.5 text-sm">
                            @error('fromDate')
                                <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        <div><label for="past-to-date" class="mb-1 block text-xs font-bold text-zinc-700">To
                                date</label><input id="past-to-date" wire:model="toDate" type="date"
                                class="w-full rounded-lg border border-zinc-300 px-3 py-2.5 text-sm">
                            @error('toDate')
                                <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <label for="past-commandant-image" class="block cursor-pointer text-center">
                            @if ($newImage instanceof \Livewire\Features\SupportFileUploads\TemporaryUploadedFile)
                                <img src="{{ $newImage->temporaryUrl() }}" alt="New commandant image preview"
                                    class="mx-auto h-40 w-full rounded-lg object-cover">
                                <span class="mt-3 block text-sm font-bold text-emerald-800">Change selected
                                    image</span>
                            @else
                                <span
                                    class="mx-auto flex size-12 items-center justify-center rounded-full bg-emerald-100 text-emerald-700"><i
                                        class="fa-solid fa-upload" aria-hidden="true"></i></span>
                            ...
                            @endif
                            <input id="past-commandant-image" wire:model="newImage" type="file"
                    ...
                    <button type="submit" wire:loading.attr="disabled" wire:target="save"
                        class="w-full rounded-xl bg-emerald-700 px-4 py-3 text-sm font-bold text-white hover:bg-emerald-800 disabled:opacity-60"><span
                            wire:loading.remove wire:target="save">Save Commandant</span><span wire:loading
