<div>
    ...
    <p class="mt-1 text-sm text-zinc-500">Manage identity, contact details, theme family, homepage slides, social
        links and maintenance mode.</p>
    <p class="mt-1 text-sm text-zinc-500">Manage identity, contact details, theme family, social links and maintenance
        mode.</p>
</div>
...
@if (session('status'))
    ...
    @if ($errors->any())
        ...
        <form wire:submit="save" class="space-y-6">
            <section class="overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-sm">
                ...
                <div class="grid gap-5 p-6 lg:grid-cols-2">
                    ...
                    <div><label class="mb-1.5 block text-sm font-semibold">Tagline</label><input
                            wire:model="siteTagline" type="text" maxlength="255"
                            class="w-full rounded-xl border border-zinc-300 px-3 py-2.5 text-sm"></div>
                    <div><label class="mb-1.5 block text-sm font-semibold">Theme Family</label><select
                            wire:model="themeFamily"
            ...
            <section class="overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-sm">
                ...
                <div class="grid gap-5 p-6 lg:grid-cols-2">
                    ...
                    <div><label class="mb-1.5 block text-sm font-semibold">Secondary Phone</label><input
                            ...
                            class="w-full rounded-xl border border-zinc-300 px-3 py-2.5 text-sm"></div>
                    <div><label class="mb-1.5 block text-sm font-semibold">Public Email</label><input
                            wire:model="email" type="email"
                            class="w-full rounded-xl border border-zinc-300 px-3 py-2.5 text-sm"></div>
                    <div><label class="mb-1.5 block text-sm font-semibold">Map URL</label><input wire:model="mapUrl"
                            type="text" class="w-full rounded-xl border border-zinc-300 px-3 py-2.5 text-sm"></div>
            ...
            <section class="overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-sm">
                ...
                <div class="grid gap-5 p-6 lg:grid-cols-2">
                    <div><label class="mb-1.5 block text-sm font-semibold">Name</label><input
                            wire:model="commanderName" type="text" maxlength="180"
                            class="w-full rounded-xl border border-zinc-300 px-3 py-2.5 text-sm"></div>
            ...
            <section class="rounded-2xl border border-amber-200 bg-amber-50 p-6">
                <div class="flex items-start gap-3"><input wire:model="maintenanceMode" type="checkbox"
                        class="mt-1">
                    <div class="flex-1">
            ...
            <div class="flex justify-end"><button type="submit"
                    class="rounded-xl bg-emerald-700 px-6 py-3 text-sm font-bold text-white hover:bg-emerald-800">Save
                    Site
                    Settings</button></div>
            <div class="flex justify-end"><button type="submit"
                    class="rounded-xl bg-emerald-700 px-6 py-3 text-sm font-bold text-white hover:bg-emerald-800">Save
                    Site Settings</button></div>
        </form>
        ...
        <div class="grid gap-6 xl:grid-cols-2">
            <section class="overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-sm">
                ...
                <div class="grid gap-3 p-6 sm:grid-cols-3"><input wire:model="socialPlatform" type="text"
                    ...
                    <div class="sm:col-span-3"><button type="button" wire:click="addSocialLink"
                            class="rounded-xl bg-emerald-700 px-4 py-2.5 text-sm font-bold text-white">Add Social
                            Link</button></div>
                </div>
                    ...
                    @forelse($socialLinks as $link)
                        <div class="flex items-center justify-between gap-4 px-6 py-4">
                            <div>
                                ...
                                class="text-xs font-bold text-red-600">Delete</button>
            ...
            <section class="overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-sm">
                <div class="border-b border-zinc-200 bg-zinc-50 px-6 py-4">
                    <h2 class="font-bold text-zinc-900">Homepage Hero Slides</h2>
                </div>
                <div class="space-y-3 p-6"><input wire:model="slideTitle" type="text" placeholder="Slide title"
                        class="w-full rounded-xl border border-zinc-300 px-3 py-2.5 text-sm"><input
                        wire:model="slideSubtitle" type="text" placeholder="Subtitle"
                        class="w-full rounded-xl border border-zinc-300 px-3 py-2.5 text-sm"><select
                        wire:model="slideImageMediaId"
                        class="w-full rounded-xl border border-zinc-300 px-3 py-2.5 text-sm">
                        <option value="">No image</option>
                        @foreach ($mediaAssets as $asset)
                            <option value="{{ $asset->id }}">#{{ $asset->id }} —
                    ...
                    <div class="grid gap-3 sm:grid-cols-2"><input wire:model="slideButtonLabel" type="text"
                            placeholder="Button label"
                            class="rounded-xl border border-zinc-300 px-3 py-2.5 text-sm"><input
                            wire:model="slideButtonUrl" type="text" placeholder="Button URL"
                            class="rounded-xl border border-zinc-300 px-3 py-2.5 text-sm"></div><button type="button"
                        wire:click="addHeroSlide"
                        class="rounded-xl bg-emerald-700 px-4 py-2.5 text-sm font-bold text-white">Add Hero
                        Slide</button>
                </div>
                <div class="divide-y border-t">
                    @forelse($heroSlides as $slide)
                        <div class="flex items-center justify-between gap-4 px-6 py-4">
                            <div>
                                <p class="text-sm font-bold text-zinc-900">{{ $slide->title }}</p>
                                <p class="text-xs text-zinc-500">Order {{ $slide->sort_order }} ·
                                    {{ $slide->is_active ? 'Active' : 'Inactive' }}</p>
                            </div><button type="button" wire:click="deleteHeroSlide({{ $slide->id }})"
                                wire:confirm="Delete this hero slide?"
                                class="text-xs font-bold text-red-600">Delete</button>
                    </div>@empty<p class="px-6 py-8 text-sm text-zinc-500">No hero slides.</p>
                    @endforelse
                </div>
            </section>
        </div>
